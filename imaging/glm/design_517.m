...
for run = 1
    ...
    fmri_spec.sess(run).cond(c).pmod(2).param       = detrend(abs(RPE_euc(ix_onset_val)) - abs(RPE_temp(ix_onset_val)),0); % vector containing parametric values
    fmri_spec.sess(run).cond(c).pmod(2).name        = 'RPE diff'; % name to call parameters
    ...
    fmri_spec.sess(run).cond(c).pmod(1).param       = detrend(abs(RPE_euc(ix_feedback_val)) - abs(RPE_temp(ix_feedback_val)),0); % vector containing parametric values
