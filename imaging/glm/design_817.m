...
for run = 1
    ...
    % other 
    trial_data = readtable(fullfile('/data/pt_02071/choice-maps/tankred_modling/data17.5.2022/RPE_data.csv'),'ReadRowNames', true);
    ...
    RPE_euc = table2array(trial_data(trial_ID,'rpeEuc'));
    RPE_temp = table2array(trial_data(trial_ID,'rpeTemp'));
    RPE_comp = table2array(trial_data(trial_ID,'rpeComp'));
    posterior_euc = table2array(trial_data(trial_ID,'posteriorEuc'));
    ml_euc = table2array(trial_data(trial_ID,'mlEuc'));
    ml_temp = table2array(trial_data(trial_ID,'mlTemp'));
    ...
    fmri_spec.sess(run).cond(c).pmod(1).param       = detrend(RPE_comp(ix_feedback_val),0); % vector containing parametric values
    fmri_spec.sess(run).cond(c).pmod(1).name        = 'prediction error'; % name to call parameters
    fmri_spec.sess(run).cond(c).pmod(1).poly        = 1; %First order polynomial expansion, always leave on 1 (so for all parametric regressors the same, could define in a loop)
