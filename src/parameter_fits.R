...
##
##  6) Lastly I compute AIC/BIC and R2, and plot these
##
#####################################################################################################################
...
temporal <- as.data.frame(read.csv("C:\\Users\\Tankred\\Documents\\Emacs\\Notebooks\\Papers to read\\rewards in mental maps\\param_fits\\sr_gp_weighted_results2.csv"))

## change to compositional without 3
compositional3 <- as.data.frame(read.csv("C:\\Users\\Tankred\\Documents\\Emacs\\Notebooks\\Papers to read\\rewards in mental maps\\param_fits\\comp_results_v3.csv"))
...
mt_euc_mod1 <- glmer(y~-1 + pred.euc + (MT - pred.euc)+ (pred.euc* trials_scaled) + ((MT - pred.euc)*trials_scaled) +(-1 + pred.euc + (MT - pred.euc) + (pred.euc* trials_scaled) + ((MT - pred.euc)*trials_scaled)|subj), family="binomial")
mt_euc_mod2 <- glmer(y~-1 + MT + ( pred.euc- MT )+ (MT* trials_scaled) + (( pred.euc- MT )*trials_scaled) +(-1 + MT + ( pred.euc- MT ) + (MT* trials_scaled) + (( pred.euc- MT )*trials_scaled)|subj), family="binomial")

cor.test(pred.euc, (pred.euc - ts_pred))
logLik(mt_euc_mod1)
logLik(mt_euc_mod2)
summary(mt_euc_mod1)
summary(mt_euc_mod2)
...
comp.output <- find.minimum(compositional3, y)
comp.output$min.nll
...
avg.mat <- ifelse(is.nan(avg.mat),0.5,avg.mat)
...
HP.euc.mat <- error.matrix[which(m.rewards> (mean(m.rewards)) + 5), ]
LP.euc.mat <- error.matrix[which(m.rewards <= mean(m.rewards)), ]
...
compute.logistic.fit <- function(matrix, excluded.trials){
  ...
  list("curve" = predictions, "inflection.point" = inflection.points)
}
...
sorted.idx <- order(m.rewards[pos.idx])
HP.error.matrix <- error.matrix[pos.idx, ]
HP.error.matrix <- HP.error.matrix[which()]
...
sr_subjR2 <- r2(successor_rep.output$min.nll)
comp_R2 <- r2(comp.output$min.nll)
euc_R2 <- r2(euc.output$min.nll)
successor_R2 <- r2(successor.output$min.nll)
MT_R2 <- r2(-logLik(MT_mod))

r2.stats <- c(comp_R2, sr_subjR2, MT_R2, euc_R2, successor_R2)

###
##### Here we compute the aic and bic weights with the best parameters. 

nlls <- c(euc.output$min.nll, comp.output$min.nll, successor.output$min.nll, successor_rep.output$min.nll, MT_nll)
...
names <- c("Euclidean", "Compositional", "SR-GP","SR", "Mean tracker")

comparison.df <- data.frame(aic.stats$weights, aic.stats$diff, bic.stats$weights, bic.stats$diff,  names)
...
p<-ggplot(data=comparison.df, aes(x=names, y=aic.stats$diff)) +
  geom_bar(stat="identity", fill="steelblue")+
  theme_minimal() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, face="bold")) + ggtitle("AIC difference") +
  xlab("Models") + ylab("Difference") +theme(axis.text=element_text(size=12), axis.title=element_text(size=14,face="bold"), plot.title = element_text(size = 18, face = "bold"))
p
