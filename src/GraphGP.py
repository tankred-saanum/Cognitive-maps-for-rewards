...
class LaplacianGP():
    ...
    def mean(self, sigma=0.01, jitter = 0.000001):
        ''' computes the posterior mean function '''
        self.inv_K = np.linalg.inv(self.K_obs + jitter*np.eye(len(self.K_obs)))

        return self.K_input_obs @ (self.inv_K + sigma *np.eye(len(self.K_obs))) @ self.y.T
