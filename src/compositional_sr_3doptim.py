...
def run_compositional_sr_model(num_samples, file_name="comp_sr_model.csv", path_integration = True):
    ...
    progress_counter = 0
    num_monsters = 12
    data_frame = np.zeros((len(subj), num_samples*num_samples*num_samples))
    states = np.arange(0, 12)


    ## start loop
    lrs = np.linspace(0.001, 0.5, num_samples)
    lengthscales = np.linspace(0.1, 4, num_samples)
    l_r, l_x, l_y = np.meshgrid(lrs, lengthscales, lengthscales)
    lengthscales = np.array([l_r.ravel(), l_x.ravel(), l_y.ravel()]).T
    header = np.arange(len(lengthscales))
    for n in range(len(lengthscales)):
        lr, lengthscale_euc, lengthscale_temp = lengthscales[n, 0], lengthscales[n, 1], lengthscales[n, 2]
        #lengthscale = 1
        last_subj = -1
        ...
        for i, subj_id in enumerate(subj):
            ...
            if subj_id != last_subj:
                ...
                sr_model = SuccessorRepresentation(states, seq_list, alpha=lr)
                SR = sr_model.get_SR()
...
run_compositional_sr_model(num_samples=15, file_name="comp_sr_model_pi_true_scale_optim_lr.csv", path_integration = True)
