...
param_df = pd.read_csv('individualHyperparameters2.csv')
tbt_weights = pd.read_csv("tBt_euc_w.csv").values
ind_lengthscales = np.array(param_df["weighted.lengthscale"])
ind_learning_rates = np.array(param_df["weighted.learning_rate"])
weights_euclidean = np.array(param_df["w.euc"])
...
for i, subj_id in enumerate(subj):
    ...
    if subj_id != last_subj:
        ...
        trial_counter = 0
        ### set hyperparameters
#        lengthscale = ind_lengthscales[subj_counter]
        lengthscale = 1.24
        ...
        if save_data:
            ...
            SR_kernel_df = pd.DataFrame(SR_kernel)
            SR_kernel_df.to_csv(f"fmri/matrices/{subj_id}/SR_kernel_matrix.csv", index=False, header=False)

            euclidean_kernel_df = pd.DataFrame(estimated_euclidean_kernel)
        ...
        m_rewards[subj_counter, trial_counter] = reward
