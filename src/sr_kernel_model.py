...
def run_sr_kernel_model(num_samples, file_name="sr_kernel_model.csv"):
    ...
    l_x, l_y = np.meshgrid(learning_rates, lengthscales)
    params = np.array([l_x.ravel(), l_y.ravel()]).T
    header = np.arange(len(params))
    for n in range(len(params)):
        lr, lengthscale_temp = params[n, 0], params[n, 1]
        #lengthscale = 1
        last_subj = -1
        print("Progress: ", progress_counter / len(params), end = "\r")
        progress_counter += 1
...
run_sr_kernel_model(num_samples=20, file_name="sr_kernel_model.csv")
