...
sim.list <- list(sr_gp_sim, euc_sim, comp_sim, MT_sim)
val.list <- list(sr_gp_val, euc_val, comp_val, MT_val)

relevant.models <- colnames(sr_gp_val)[1:4]
...
for (i in 1:length(sim.list)){
  ...
  nll <- compute.validation.scores(y, val.data, relevant.models)
