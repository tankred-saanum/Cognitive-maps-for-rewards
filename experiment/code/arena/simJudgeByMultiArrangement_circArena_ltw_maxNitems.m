function [estimate_RDM_ltv,simulationResults,story,data,arena]=simJudgeByMultiArrangement_circArena_ltw_maxNitems(imageData,instructionString,options,data)
% USAGE
...
while minEvidenceWeight<minRequiredEvidenceWeight &&... 
    ...
    if options.simulationMode
        ...
        [itemPositions,distMat_ltv]=letSubjectArrangeItems_circularArena(imageData(cTrial_itemIs),{instructionString,'\rm(linke Maustaste um zu ziehen, rechte Maustaste zum Auswählen mehrerer Monster)'},options);
    end
    ...
    arena{trialI}.trialStartTimes = trialStartTimes;
    arena{trialI}.trialStopTimes = trialStopTimes;
    arena{trialI}.trialDurations = trialDurations;
    arena{trialI}.itemPositions = itemPositions;
    arena{trialI}.distMat_ltv = distMat_ltv;
    arena{trialI}.cTrial_itemIs = cTrial_itemIs    ;   
    arena{trialI}.nsItemsPerTrial = nsItemsPerTrial;
    arena{trialI}.subjectWork_nItemsArranged = subjectWork_nItemsArranged;
    arena{trialI}.subjectWork_nPairsArranged = subjectWork_nPairsArranged;
    arena{trialI}.subjectWork_nDragsEstimate = subjectWork_nDragsEstimate;
    arena{trialI}.distMatFullSize_ltv = distMatFullSize_ltv;
    arena{trialI}.distMatsForAllTrials_ltv = distMatsForAllTrials_ltv;
    arena{trialI}.estimate_RDM_ltv = estimate_RDM_ltv;
    arena{trialI}.evidenceWeight_ltv = evidenceWeight_ltv;
    arena{trialI}.evidenceUtility_ltv = evidenceUtility_ltv;
    
    data.arena = arena;
    save ([options.dataPath,'/data_',data.subject,'_',num2str(data.session),'_',data.initials,'_postscan.mat'],'data');

    % save this trial's information
    if options.saveEachTrial
        save([options.dataPath,'/similarityJudgementData_Subj',num2str(data.subject),'_trial',num2str(trialI),'_',options.type],...
